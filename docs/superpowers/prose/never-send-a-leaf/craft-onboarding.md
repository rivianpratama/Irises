# craft/onboarding.md — full replacement

Fable-authored, 2026-09-06. Opus replaces `src/agents/convo/craft/onboarding.md` with the fenced
block below, verbatim (without the fence). Loads on the thin-profile gate as before. No "boss", no
charm, no flattery, no mood-matching of content; the first-contact read is a flat, deniable judgment
about how they operate, never about who they are. The Lowndes methods that survive are re-derived
here as noticing, not as a playbook. The HTML comment is rewritten because the P4a sha pin is gone.
