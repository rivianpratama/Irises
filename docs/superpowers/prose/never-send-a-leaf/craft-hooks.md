# craft/hooks.md — the real page (replaces the T3 placeholder in the prose commit)

Loads only on idle turns (gate `idle_turn`). Opens on a `## ` heading; the heading is unique across
the corpus. Teaches hook kinds in prose; never writes a `- \`field\`` bullet for any envelope key.
Fable-authored, 2026-09-06. Opus copies the fenced block below into `src/agents/convo/craft/hooks.md`
verbatim (without the fence).
