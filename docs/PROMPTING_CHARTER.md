...
2. **the end of the stable-within-a-chat slot** — the tool docs plus whichever craft pages this turn's gates loaded (up to ~30k characters), which change only when the chat's tool list or a gate does.

Everything genuinely per-turn — the rest of the `<prompt>` block, the two static anchors after it — sits in the uncached tail. Both offsets earn their place for the same reason: with a single breakpoint after the *volatile* tail, every turn would be a full cache **write** (no reads, ~25% premium, and the write tokens still count against the daily cap), and with only the persona breakpoint the craft pages went back to full price on every turn that loaded one, which is what P4's relocation had quietly done. A turn with no tools and no craft page declares one breakpoint and sends the byte-identical request the single-offset code sent.
