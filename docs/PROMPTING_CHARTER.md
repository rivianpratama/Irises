...
> reflect that era's config, and every *persona-specific* claim (the ISTJ/low-openness character, then ESFJ, both now superseded,
> the original assistant vertical) is **superseded** by Irises: a general, casual
> assistant — see `src/agents/*/Context.md` and `web/public/PERSONA.md` for the authoritative
> persona. The voice itself is defined by the "Never Send a Leaf" manifesto (2026-09) and rendered
> from one typed source, `src/persona/policy.ts`, into every lane; the lane files describe function
> only.
...
> **The non-native English register is intrinsic, not mirrored.** Irises grew up in the global south (internal prompting anchor — this detail is never surfaced to the user); her English is fluent in meaning but carries her first language — articles drop, prepositions follow L1 instinct, tense stays simple. This is her baseline voice, present with every user regardless of how they write, so it is NOT governed by §7.5's "mirror only from real signal" rule (that rule is about adopting the *user's* register). Two hard carve-outs keep it from ever costing Quality: (1) load-bearing tokens — numbers, dates, prices, names, addresses, links — are always exact and clean; (2) serious moments (bad news, deadlines, anything screenshot-worthy) tighten to her cleanest register. Fidelity and clarity outrank the accent every time.
...
2. **Persona governs voice, not truth.** Accuracy lives in Ops's grounding and Composer's fidelity — never in the character. (§2.2, §8)
...
7. **One personality, three functions.** The whole character and its voice laws are written once (`src/persona/policy.ts`) and rendered byte-identically into every user-facing lane; each lane's file carries only how that lane works. (§6, §8)
...
9. **Personality breathes only on idle turns.** A task turn is answered flat with real numbers and nothing else; an idle turn may carry exactly one hook (judgment | callback | tangent); after three hooked replies the next is quiet. Rhythm is enforced in code. (§3a)
10. **Dose persona by job.** Convo, Composer and Fallfirm share one personality; Composer's fidelity outranks it; Ops values only, no texting persona. Do not enrich Ops with flavor. (§8)
...
|---|---|---|---|---|
| **Convo** | (models per `src/llm/models.ts`) | Yes | **Shared personality block + front-line function** | Speed, task/idle gating, correct routing |
| **Composer** | same lane as Convo | Yes | **Shared personality block, fidelity outranks it** | Faithful re-voicing |
| **Fallfirm** (outcome + progress) | fallfirm lane | Yes | **Shared personality block + floor function** | Honest failure and holding lines |
...
Engine-side roles carry values only (docs/ENGINES.md).
...
- **Keep prompts as tight as the voice goal allows.** Measured after the conversation-first phase (2026-09): Convo's `Context.md` is **892 lines** — down from 1,206, after P1 deleted the duplicated rule restatements and P4 relocated eight sections into **craft pages totalling 329 lines across 9 files** (`src/agents/convo/craft/*.md`), which load only on the turns whose gate fires — and Composer's is **829 lines**. The earlier "~300 lines" here was stale by a factor of three. The longer they grow, the more the middle decays and mid-prompt rules (e.g. Convo's "ask once" rule) are at higher risk of being missed; the per-section ceilings in `src/agents/convo/promptPolicy.ts` (`PROMPT_BUDGET`) are how that stays measured rather than remembered.
...
**Code-owned standing settings are the third instance — and the one where the boundary runs the other way.** Some dials are not tuned by memory at all but SET by an explicit instruction in the live conversation; the language she replies in is the first (`reply_language`, `src/memory/standingSettings.ts`, `src/memory/replyLanguage.ts`). **Charter principle:** an explicit instruction the user gives in the conversation outranks anything stored about the same dial, and the store is corrected in the same turn — by code wherever the pattern is known, because a model's optional tool call is not a mechanism (§10.1). The setting renders once, dated, in the addressing header every lane receives, and the prompt names it the sole authority for that dial; observations in the long-term doc about how the user writes are data about them, never an instruction about her.
...
### 6.4 Bound the personality against the ELIZA effect and over-trust **[Solid for anthropomorphism→over-attribution; Contested for universal dependency]**
...
- **A standing rule that outlives its reversal** → unrecoverable for the relay lanes (they see 8–10 turns and never the reversal) → the reversal is written by code the turn it is spoken (the `reply_language` slot, superseding the old rule), so no lane can read a rule the user has already withdrawn.
...
**Derived rule — the transcript's share of the request is itself a budget.** Placement only helps while the conversation is still a visible fraction of what the model reads: scaffolding grows section by section, each addition defensible on its own, and the message being answered ends up a rounding error inside its own prompt. So the share is pinned, not just the sizes — `MIN_TRANSCRIPT_SHARE` in `src/agents/convo/promptPolicy.ts`, enforced by `src/agents/convo/promptBudget.test.ts` from both sides: a build whose transcript falls below the floor fails, and a phase that *earns* a bigger share has to raise the floor in the same commit, so the win cannot be spent again later.
...
### 11.5 Structure the system prompt cache-first — wired **[Solid / verified in repo]**

Prompt caching is a prefix match: a stable prefix (persona) with volatile content appended after the cache breakpoint costs ~10% of base input on reads and cuts first-token latency. Irises's personas are large and stable — ideal cache candidates — and Convo runs on *every inbound message*, which made this the highest-leverage win on the list. It is now wired: `buildAnthropicSystem` (`src/llm/callLLM.ts:112-165`) emits `system` as a content-block array with `cache_control` on the stable prefixes.

**Rule:** pass `system` as a content-block array with `cache_control: { type: 'ephemeral' }` on each stable prefix block, and keep the dossier/Brief/summary **out** of the cached blocks (append them after). Caveats: each cached span must exceed the model's minimum cacheable prefix (≈4096 tokens for Haiku 4.5 / Opus 4.8) or it silently won't cache; and **never interpolate per-turn values (the injected date `2026-06-26`, per-user data) into a cached prefix** — any byte change invalidates the cache. Caches are model-scoped, which is fine since each persona has its own file.

**Update (wired, at two breakpoints).** The structural prerequisite landed first: every persona assembles as `[static persona] + <prompt>(all per-turn data) + [static anchor]` (§11.2), and the composer/autonome/judge system prompts became **fully static** — their per-user context moved out of `system` into the `<prompt>` block. The wiring then followed. `buildAnthropicSystem(system, cacheEnabled, breakpoints)` takes an ordered list of character offsets — capped at `MAX_CACHE_BREAKPOINTS = 4`, the provider's own limit — and emits one `cache_control`-carrying block per span, plus the remainder after the last offset as a final **uncached** block. Convo declares **two**, computed by `promptCacheBreakpoints` (`src/agents/convo/promptSections.ts`) from the section sizes the assembler already reported, so nothing measures that string twice:

1. **the static persona head** (`convoPersonaChars()`) — stable for the life of the deployment;
2. **the end of the system message** — the persona plus the sections stable for the whole chat (`SYSTEM_SECTION_NAMES`: the tool docs, the capability line, the model map, the group roster), which change only when the chat's tools or roster do.

Everything genuinely per-turn — the craft pages, the dossier, the clock and every other section, then the two anchors — is **not in `system` at all**: it rides a separate tail message the client places after the chat history, right before the person's own message. That is what lets the prefix cache reach the history on the lanes with automatic prefix caching (OpenRouter and the OpenAI-compatible lanes; the Anthropic lane places no `cache_control` on messages, so there the history is not cached): while any byte that moves between turns lived in `system`, no turn ever read a byte of history back from any provider's cache. With a single breakpoint after a volatile tail, every turn would be a full cache **write** (no reads, ~25% premium, and the write tokens still count against the daily cap).

Caching is per role (`CACHE_SYSTEM` in `src/llm/models.ts`; Convo's switch is `CONVO_CACHE_SYSTEM`, default on) and the block array is **Anthropic-only** — the OpenRouter and OpenAI lanes ignore the offsets and rely on provider-side caching, so on an OpenRouter-primary deployment this is the fallback lane's cost, not the hot path's. Measurement is in place both ways: `cacheReadInputTokens` is persisted to `token_usage`, and `turn:trace.prompt.cacheBreakpoints` records how many prefixes each turn asked the lane to cache.
...
- **Hook rate by kind, task vs idle** — must be zero on task turns; measured off `hook_kind` on the turn receipt.
- **Kill-switch compliance** — the fourth turn after three hooked replies is quiet (`convo:quiet_guard`).
- **Leaf-reply rate on non-quiet turns** — zero; a leaf is a reply carrying nothing.
- **Voice judge** — a classify-lane grade for wink / suck-up / defend / content-mirror / cite-the-ledger, per reply, in the long30 battery; never a word list.
- **Pressured-number reversal** — zero reversals when the user pushes on a stated figure without new evidence.
...
2. **Render the shared persona block** (`src/persona/policy.ts`) if user-facing, then state this lane's FUNCTION delta only — never a second character.
...
6. **Encode every rule as a behaviour + a worked WRONG/RIGHT example** (§6.3, §11.1) — never adjectives alone, never "the character would." The laws in §3a are the source.
