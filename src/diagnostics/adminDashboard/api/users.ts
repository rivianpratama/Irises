...
// User roster: every known handle (profiles ∪ live activity), resolved to
// names, with live activity and turn counts — the drill-in hub that links
// each user to orchestration / memory / history / llm.
  ...
  router.get('/dashboard/api/users', async (req: Request, res: Response) => {
    ...
    try {
      const base = await cached('users:base', 30_000, async () => {
        const [profiles, historyKeys] = await Promise.all([
          listUserProfiles(),
          listHistoryKeys(1000),
        ]);
        return { profiles, historyKeys };
      });
