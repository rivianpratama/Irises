...
// openclawOnboarding.ts). OpenClaw itself is never modified and never hand-edited — the doctrine
// travels the only sanctioned way, as a chat message the engine saves by its own hand, exactly like
// the manual hermes send documented in bridge/hermes/engine-onboarding-message.md.
//
...
/**
 * The standing section, sent once per content version. Adapted from the hermes onboarding message
 * (bridge/hermes/engine-onboarding-message.md) for this gateway: the ask is replace-by-heading so a
 * re-send is idempotent, memory is per-AGENT rather than per-chat, and the two OpenClaw-specific
 * sections are the capability invitation and the never-message-the-user limit (this engine HAS its
 * own channels to the user, so the limit is a live hazard rather than a formality).
 */
