...
//
// GOLDEN_BLOCK_C has been regenerated ONCE too, for the same phase's second half: the weather block's
// 470-character re-report tail became one pointer line, and the `status_contract` section it points
// at (persona/status.ts renderStatusContract) now sits between the weather block and the thread
// block. Those two are the whole diff on that line — the envelope's field list moved out of the
// persona and into generated prose, so the bytes moved with it.
//
