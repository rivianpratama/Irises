...
test("the anchor's bubble guidance is interpolated from the pipeline constants", () => {
  ...
  assert.ok(
    anchor.includes(`Aim for ${BUBBLE_WORD_TARGET_LO}-${BUBBLE_WORD_TARGET_HI} words per item`),
    'the guidance sentence reads off the word-target constants',
  );
  assert.ok(
  ...
  // The bubble paragraph has only the target-band digits and "1-2" (typical replies), no leftover
  // hard caps or ceiling numbers.
  const law = anchor.slice(anchor.indexOf('Each item in `bubbles`'), anchor.indexOf('No markdown'));
  const allowed = new Set([BUBBLE_WORD_TARGET_LO, BUBBLE_WORD_TARGET_HI, 1, 2]);
