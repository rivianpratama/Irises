...
// nothing else moved. Every regeneration was produced by dumping `stable(afterPersona(...))` for
// fixture C out of the live assembler and splicing it into the literal programmatically — never
// typed by hand — then diffed opcode by opcode (difflib) against the literal it replaced, which is
// where those insert-counts come from.
//
