...
export const PROMPT_BUDGET: Record<BudgetKey, number> = {
  ...
  behavior_anchor: 928,        // 910 — the DRIFT ANCHOR (persona/policy.ts renderDriftAnchor). The Never-Send-a-Leaf rewrite: +211 from 699, and the shape changed with the size — it is rendered per turn now instead of the static string it replaced, three identity bullets picked by the transcript window and three stating the law for THIS turn's mode. Still six bullets and still digit-free, so what moved is what the six SAY. Six variants, measured: task 726 short / 910 long, hook 743 / 927, quiet 698 / 882. The FIXTURES reach 910, on the sixth fixture — the mature turn on the eighty-row window the box ships (deploy/app.env CONVO_HISTORY_MAX=80), the only one past DRIFT_LONG_WINDOW_CHARS — and this ceiling stands 2% over that, the most the ratchet in promptBudget.test.ts allows. The MODE dimension USED to cost nothing further, because task stated the longest of the three laws in both bands. It no longer does: the hook bullet took "A hook is a statement, never a question." (+41), so hook/long is now the WIDEST variant at 927 — one character under this ceiling. No fixture renders a hook-mode long window, so the ratchet still measures 910 and the ceiling cannot be raised to follow 927 without breaching the 2% band. The next edit that lengthens a hook bullet has to shorten one of the six first, or give the fixtures a hook-mode long turn to measure and move the ceiling honestly. The WINDOW is the dimension that moves it, which is why a fixture had to reach the band rather than leave it as a note here — this key is enforced live (convergence/focusBattery.ts prose_budget scores every PROSE budget key), so a maximum measured only on short windows is not a paper debt, it is a breach on the first long thread. Was 730 for 726, task/short, the only band any fixture then reached. Was 705 for 699 — P1: six lines that drift first (was 1,740 for 1,659 / 14 lines)
