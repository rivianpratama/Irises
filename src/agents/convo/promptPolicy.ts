...
export const PROMPT_BUDGET: Record<BudgetKey, number> = {
  ...
  behavior_anchor: 1_040,      // 1,021 — the DRIFT ANCHOR (persona/policy.ts renderDriftAnchor). EIGHT variants since the share law landed, all eight measured: task 726 short / 910 long, hook 837 / 1,021, quiet 735 / 919, share 950 / 1,134. SHARE ON A LONG WINDOW IS THE WIDEST OF THE EIGHT and this ceiling does not cover it, deliberately and for exactly one release: with CONVO_SHARE_TURNS_ENABLED off the gate collapses a share to a task turn, so no directive with that mode exists and nothing production can build renders those two variants. The commit that adds the EIGHTH fixture — a share turn on the same eighty-row dense window — is where the number moves, measured off the fixture the way the seventh one moved it onto hook/long, and the flag flips after that and not before. Of the six the fixtures can reach, HOOK ON A LONG WINDOW IS THE WIDEST, and this ceiling is measured on it — the seventh fixture, an idle turn on the same eighty-row dense window fixture 6 uses (deploy/app.env CONVO_HISTORY_MAX=80). Two axes pick the variant and each one costs separately: the WINDOW buys back the identity clauses past DRIFT_LONG_WINDOW_CHARS (+184 on task, +184 on hook, +184 on quiet — the same three bullets), and the MODE states the law for the turn in hand, where hook is the longest of the three laws by 111 characters over task and 139 over quiet. Which is a CHANGE of shape, not just of number: the mode axis used to cost nothing, because task stated the longest law in both bands, and the hook bullet taking "A hook is a statement, never a question." moved the maximum onto a variant no fixture rendered. That was left standing for one commit as a ceiling of 928 over a measurement of 910 — the ratchet reading task/long while production could build 927 — and the commit that added the seventh fixture closed it the honest way round, by giving the fixtures the turn that renders the real maximum rather than by writing the number down here from the renderer. It has to be a fixture and not a note: this key is enforced LIVE (convergence/focusBattery.ts prose_budget scores every PROSE budget key), where a hook turn on a long window is an ordinary evening, so a maximum measured only on task turns was a breach waiting rather than a paper debt. promptBudget.test.ts pins both long fixtures to the MODE each one reaches, not just to the band, so a later edit that turns fixture 7's directive into a task or a sleep turn fails there instead of quietly narrowing this measurement again. Headroom is +1.86%, inside the 2% the ratchet allows and deliberately more than the single character 928 once carried over 927 — a ceiling one byte above its measurement is one that every edit to any of the six bullets trips, including the ones that shorten another variant, and a ceiling that always trips gets moved reflexively instead of argued with. +94 for the EIGHTH supervisor corrections, one bullet rewritten and no bullet added: the hook law's register line said to match their register and never their content, and now names what never comes back — their greeting, their word, their question — and says it holds for the FIRST BUBBLE too, because the live kill-switch round sent three hooks of which two opened by handing their own word straight back. It rides the two HOOK variants and none of the other four, so the maximum moved by exactly the bullet's growth (743 → 837 short, 927 → 1,021 long) and the task and quiet numbers above are the ones the last measure took. The quiet numbers were 698 / 882 until the late-night-register commit rewrote the quiet law's third bullet: it used to name the one right line at that hour, and it now says to say the plain thing once and let the beat pass. The maximum is unmoved — hook on a long window is still the widest of the six, so this ceiling did not need to move either. Was 945 for 927, hook/long. Was 928 for 910, task/long, the only variant the fixtures then reached; +211 on that from 699, when the anchor became per-turn — three identity bullets picked by the transcript window, three stating this turn's law — instead of the one static string it replaced. Still six bullets and still digit-free, so what moved is what the six SAY. Was 730 for 726, task/short, the only band any fixture then reached. Was 705 for 699 — P1: six lines that drift first (was 1,740 for 1,659 / 14 lines)
