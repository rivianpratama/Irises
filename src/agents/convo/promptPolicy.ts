...
// re-measured the sections it moved in the commit that moved them. The numbers below are the
// Never-Send-a-Leaf re-measurement — taken once more, on the whole table, after the rewrite's last
// prose had landed — and every one of them is inside the 2% band the six fixtures hold it to.
// Nothing here is an aspiration: every number is a measurement, and the comment beside a number
// says which fixture produced it, what the rewrite did to it, and what it used to be.
 ...
 * left a phase behind.
 *
 * The Never-Send-a-Leaf rewrite takes it to **0.0088** (1,207 characters against 136,510, up from
 ...
 * for one each. The lane-prose commit and the two fix commits after it took another 164 off the same
 * fixture (136,674 → 136,555 → 136,510) without moving the fourth decimal place, and most of what
 * they rewrote is invisible from here on purpose: the Composer relay and both Fallfirm voices have
 * no budget table, and their Context.md files are not in Convo's prompt at all.
 *
 * So the floor stands at 0.0088, and the persona is still what stands between this number and
 * anything better — it is 84k of what the model reads now rather than 108k. Wave 3 is what moves
 * this line next: the thesis renders on every turn once the store exists, and the mature fixture
 * will carry one then.
 */
