...
// touched. So every number here is inside the 2% band the six fixtures hold it to, and every one of
...
export const PROMPT_BUDGET: Record<BudgetKey, number> = {
  ...
  hooks: 1_685,                // 1,653 — THIS TURN'S RHYTHM CONTRACT (persona/hooks.ts renderHooksSection), NEW in the Never-Send-a-Leaf rewrite, second-to-last in the block and immediately ahead of the turn-focus read. Measured on the two idle fixtures, which render the widest variant the engine can build TODAY: heading, lead, the open-kinds line with all three kinds named, and the SLEEP LINE. That line is not a Wave 3 thing and never was — compileAffect sets `sleepQuiet` straight off the circadian slot (persona/affectCompiler.ts, `dead_night`/`pre_sleep`: no flag, no store, no mood in it), the selector passes it through in hook mode, and the renderer pushes it. So a hey at two in the morning — the turn this feature is named after — renders 462, which is why the fixtures' frozen clock (02:00 UTC, `dead_night`) is the one this ceiling is measured on. Every other variant is smaller: hook without sleep 338, quiet 321 (the quiet law already says "it is late for them", so the sleep line is not added there), no kind open 291, no kind open plus sleep 415. It was briefly ceilinged at 344 for the 338 variant — a shape that clock cannot produce — and this key is enforced live (convergence/focusBattery.ts prose_budget scores every PROSE budget key), so that was not a paper debt but a breach waiting for the first late-night idle turn. THE MOMENT LEAD IS NOW BEHIND IT, and it is what moved this line: with `directive.moments` open the renderer pushes the lead plus the sampled lines, and the sample is DATA inside a section whose ceiling the live battery enforces as prose. So the fixture renders the widest sample the engine can build rather than a plausible one — five lines (`MOMENT_SAMPLE_RECENT + MOMENT_SAMPLE_OLD`, the most `sampleMoments` ever hands a turn), each at `MOMENT_TEXT_MAX`, which `renderMomentLines` clamps to at the render seam whatever the file holds, each wearing the longest tag and the longest of the seven age phrases — and promptBudget.test.ts asserts that shape so it cannot silently narrow. 1,777 is that sample on the thread-offer fixture; the two moment-less idle variants still measure 462 (with the sleep line) and 338 (without). ZERO on a task turn, which is the no-regression pin the whole feature rests on: on the turns that are real work this section does not exist and the prompt is byte-identical to an install that never had a hook engine. RE-MEASURED at the end of the phase and unmoved at 1,777, which is the rule working rather than a coincidence: the sampler and this ceiling landed in the same commit, so the final audit had nothing left to find on the one line the earned material can reach. Was 470 for 462, before a sampler existed to fill the moment lead. Was 1_810 for 1,777 — −124 at the whole-branch review, the sleep line and its newline, and the reason is a correction rather than a deletion. `sleepQuiet` used to be ADDITIVE: the section named all three kinds and then said to send them to bed, while the drift anchor at the recency edge said "one hook" — three copies of one turn disagreeing, on the plan's own 2am case. A late idle turn is now a CLOSED-KINDS hook turn (persona/hooks.ts): every kind forbidden, no moments, no thread offer, so it renders heading + lead + the none-open line + the sleep line + the clamp, and the anchor's mode moves to quiet with it. The widest variant the engine can build is therefore the ordinary DAYTIME idle turn — all three kinds open, the spacing interval spent, the widest sample — because it is the only shape that can carry the moment lead at all: the sleep branch shuts the sampler, a moment only ever rides out as a callback, and sampling bills it either way. That fixture's clock is still 02:00, and the directive is HANDED to the assembler rather than compiled inside it (promptBudget.test.ts DAYTIME_HOOK says so), because production has twenty-four hours in the day and a ceiling taken on the widest producible shape is the honest one. The moment-less variants now measure: hook without sleep 338, sleep 415, quiet 321.
  ...
  behavior_anchor: 928,        // 910 — the DRIFT ANCHOR (persona/policy.ts renderDriftAnchor). The Never-Send-a-Leaf rewrite: +211 from 699, and the shape changed with the size — it is rendered per turn now instead of the static string it replaced, three identity bullets picked by the transcript window and three stating the law for THIS turn's mode. Still six bullets and still digit-free, so what moved is what the six SAY. Six variants, measured: task 726 short / 910 long, hook 702 / 886, quiet 698 / 882. This ceiling is the widest of them, on the sixth fixture — the mature turn on the eighty-row window the box ships (deploy/app.env CONVO_HISTORY_MAX=80), the only one past DRIFT_LONG_WINDOW_CHARS. The MODE dimension costs nothing further: task states the longest of the three laws in BOTH bands, so hook and quiet fit under a ceiling measured on task. The WINDOW is the dimension that moves it, which is why a fixture had to reach the band rather than leave it as a note here — this key is enforced live (convergence/focusBattery.ts prose_budget scores every PROSE budget key), so a maximum measured only on short windows is not a paper debt, it is a breach on the first long thread. Was 730 for 726, task/short, the only band any fixture then reached. Was 705 for 699 — P1: six lines that drift first (was 1,740 for 1,659 / 14 lines)
  json_anchor: 3_000,          // 2,975 — the envelope contract, last in the prompt, the same on every fixture. The Never-Send-a-Leaf rewrite: +47 from 2,928, for the one clause the closing `status` paragraph owes the rhythm engine — the recency edge now names the extra beat a reply may have carried, so the field the contract describes is also asked for in the last thing she reads. This is also where the envelope contract now stands ALONE: the **ABSOLUTE RULE** blockquote that opened Context.md said the same four fields in the same order and came out with the character sections (see `persona`). Was 2_950 for 2,928 (+20 in P3: the one-line description of `status` stopped asking for the gauges the schema no longer carries). Was 3,050 for 2,908
  memory_stack: 6_350,         // 6,275 — the mature stack, card through long doc, on a turn that touches none of it; the wrapped tiers inside `context_block` (memory/wrappers.ts). The Never-Send-a-Leaf rewrite: −3 from 6,278, the same three characters `context_block` records, because this stack is where they are — the addressing rule, the seed stance and the style-defaults ladder are all re-authored here. The ceiling stands at 1.2% headroom. Was 6,278 — +305, the same two edits `context_block` carries: law (b)'s reply-language clause and the `(since <date>)` suffix on every standing rule. Was 6_050 for 5,973 (+6 when law (b) started naming the layers that are really in the prompt). Was 5,950 for 5,837; +130 gave the medium tier back its hard-personal-rules line. Was 12,500 for 12,290; the card is +43 on the preamble it replaced and absorbed the tenure section, then the three ladders and the three seed stances came off
};
 ...
 *
 * So the floor stands at 0.0087, and the persona is still what stands between this number and
 * anything better — it is 84k of what the model reads now rather than 108k.
 *
