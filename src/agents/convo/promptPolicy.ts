...
export const PROMPT_BUDGET: Record<BudgetKey, number> = {
  ...
  hooks: 344,                  // 338 — THIS TURN'S RHYTHM CONTRACT (persona/hooks.ts renderHooksSection), second-to-last in the block and immediately ahead of the turn-focus read. Measured on the two idle fixtures, which render the widest hook-mode variant this commit can build: heading, lead, and the open-kinds line with all three kinds named. The other variants are smaller (a quiet block is three lines; a forbidden kind shortens the list), and the two that are not yet reachable — the sleep line, and a moment lead with sampled lines under it — arrive with Wave 3's sampler and re-measure this line then. ZERO on a task turn, which is the no-regression pin the whole feature rests on: on the turns that are real work this section does not exist and the prompt is byte-identical to an install that never had a hook engine
