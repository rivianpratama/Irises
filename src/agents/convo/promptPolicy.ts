...
//
// Since P4a the persona is two things, and the numbers below distinguish them: the always-on head
// (`persona` — the shared persona block from persona/policy.ts, then Context.md) and the craft pages
// that load per-turn (`craft_modules`, convo/craft/*.md — up to 23k of them on the turn that needs
// the most). A phrase check reads BOTH,
...
export const PROMPT_BUDGET: Record<BudgetKey, number> = {
  ...
  tool_docs: 18_350,           // 18,206 — the group fixture. −113 in the prose commit, spread over five docs and one deletion: the delegate holding text and the schedule confirmation are flat rather than warm (and the holding doc stops explaining where her warmth lives), the antisocial-hour nudge became a plain statement that sets the thing anyway, cancel_automation stopped leaving the door open, update_directives declines flat, and update_memory lost the congratulations example whose whole point was a beat she no longer sends. The ceiling stands: it was measured at 18,319 and the section only shrank, so the headroom is 0.8% and inside the band. Was 18_350 for 18,319 (14 tools; the 1:1 lane carries 12). +688 for the reply-language slot: set_preference gains the `reply_language` clause and its key list the slot's name, and update_directives now REFUSES a language ask (it names set_preference instead of the old "add always reply in Spanish") and gains the change-your-mind line that stops two rules standing that disagree — the tool-side half of the 2026-09-04 failure, where the only mechanism was a call the model never made. Was 17_650 for 17,631. +1,185 for steer_research, the sibling of cancel_research that adds to a run instead of dropping it (tools.ts): its doc is what stops the model spending a live run on a "also check X" — a fresh delegate_to_ops or a cancel. Was 16,450 for 16,446 (13 tools; the 1:1 lane 11). +201 for delegate_to_ops' `effect` arg (the approval gate's read/act tag, tools.ts) — the ceiling was already 16,450 and stands, so this line is now measured to four characters. Was 16,245; was 16,500 for 16,288, before remember_user's `handle` doc stopped asking "whose info this is" and named the messaging handle instead
  capability: 240,             // 237 — all six capability classes minus inbox, the longest line (was 248)
  model_map: 800,              // 384 on a bare checkout — HOST-DEPENDENT, see above. −3 in the lane-prose commit: the tail stopped asking her to keep the model answer a light sentence and swing back to them, and now says two model names, one flat sentence, then stop. Was 387
  name_nudge: 173,             // 170 — fixed prose. +7 for the address rule's sixth and last copy: the nudge used to hand her a placeholder nickname for the gap ("call them \"boss\" for now"), and the rule is now that the gap gets NOTHING — a saved address_as, else their name, else no address term at all. Spelling "address them as nothing" costs more characters than naming a word, which is the whole trade this line records. Was 165 for 163 (was 171)
  intro_weave: 790,            // 779 — INTRO_WEAVE_BLOCK (agents/ops/firstMove.ts). +19 in the prose commit: the block's two personality clauses are re-authored on the mechanisms the shared block states — "mood match FIRST" becomes register-matched-never-content (a task opener gets the task first), and the "ONE light association" the read was allowed to colour the reply with becomes ONE flat, deniable judgment. Same single-beat ceiling, same never-a-word-about-being-installed clause, and the craft pointer now names the page rather than the deleted section. Was 770 for 760 (was 795)
  context_block: 7_500,        // 7,451 — −3 in the prose commit, which is what the address rule's third copy costs after the two long clauses either side of it cancel out (the no-address-term rule is longer than the nickname it replaced; the stance seed and the style-defaults ladder are shorter in the new register). The ceiling stands at 0.7% headroom. Was 7,454 — +305 for the reply-language law and the dates on the standing rules: law (b) now names the Reply language line in the addressing header as the SOLE authority on the language she replies in (a rule the relay lanes could read at all, unlike the reversal they never saw), and every standing rule renders with the day it was asked for, which is what makes "trust the newer entry" an instruction a lane can act on. Was 7_200 for 7,149 — the thread-offer fixture: the mature stack (5,973) plus BOTH plain sections a routed block can carry, a steering question and an approval ask outstanding at once (memory/dossier.ts). Was 6,700 for 6,623 with the steering question alone; the approval gate's section is the +526. Was 6,050 for 5,967 while no fixture carried a plain part at all, and 5,950 for 5,837 before the medium tier took back the hard-personal-rules line the ladder deletion dropped with nowhere to land. Was 12,500 for 12,290; P2 sent the discovery scaffold's craft coaching to craft/onboarding.md, gated every memory block on the turn, folded the plain tenure section into the identity card, replaced each tier's You-should/You-MUST-NOT ladder with the two or three lines that tier alone decides, and retired the three seed stances
  thesis: 380,                 // 373 — HER ONE READ on this person (memory/thesisEngine.ts), rendered right behind the dossier it is the conclusion of. A DATA line, not a prose ratchet: the text is written per person by the weekly pass and its length is a property of them, which is why it sits with `context_block` in convergence/expectations.ts and is reported rather than failed on a live overshoot. Measured on the thread-offer fixture, whose thesis is the length the writer prompt asks for (two to four sentences, behaviour only). It renders on no other fixture and, until Wave 3 builds the store, on no live turn either: `personaTurn.thesis` is `''` from convo/client.ts, and '' pushes nothing at all
  active_ops: 2_600,           // 2,570 — two looks in flight, one queued, one of them carrying a mid-run addition of theirs. +624 is run control's own half of this section: the steer/redirect instruction block (a steer keeps the run, a replacement is cancel + delegate in one turn) plus the "you added" suffix the addition renders on its status line. Was 1,980 for 1,946 with the stop instruction alone (was 2,040)
  ...
  hooks: 470,                  // 462 — THIS TURN'S RHYTHM CONTRACT (persona/hooks.ts renderHooksSection), second-to-last in the block and immediately ahead of the turn-focus read. Measured on the two idle fixtures, which render the widest variant the engine can build TODAY: heading, lead, the open-kinds line with all three kinds named, and the SLEEP LINE. That line is not a Wave 3 thing and never was — compileAffect sets `sleepQuiet` straight off the circadian slot (persona/affectCompiler.ts, `dead_night`/`pre_sleep`: no flag, no store, no mood in it), the selector passes it through in hook mode, and the renderer pushes it. So a hey at two in the morning — the turn this feature is named after — renders 462, which is why the fixtures' frozen clock (02:00 UTC, `dead_night`) is the one this ceiling is now measured on. Was 344 for 338: hook mode with the fixture's `sleepQuiet` switched off, a shape that clock cannot produce, and this key is enforced live (convergence/focusBattery.ts prose_budget scores every PROSE budget key), so it was not a paper debt but a breach waiting for the first late-night idle turn. Every other variant is smaller — hook without sleep 338, quiet 321 (the quiet law already says "it is late for them", so the sleep line is not added there), no kind open 291, no kind open plus sleep 415. What DOES still wait for Wave 3 is the moment lead with sampled lines under it (`directive.moments`, no sampler behind it yet), and it re-measures this line when it arrives. ZERO on a task turn, which is the no-regression pin the whole feature rests on: on the turns that are real work this section does not exist and the prompt is byte-identical to an install that never had a hook engine
  turn_focus: 550,             // 544 — a 400-char restatement plus two hits, on the mature fixture, which is a TASK turn and renders `Turn: task`. Two fixtures are idle now and both render the longer form (`Turn: idle · their message: 43 characters · 12th idle in a row`), and neither is the maximum: the idle turns here are a three-character "hey" and a forty-three-character stall, and the restatement is what dominates this line. So the debt the last line recorded turns out to be smaller than it read — an idle turn adds about sixty characters to the SHORTEST restatements there are. The number is unmoved and is now measured on fixtures that exercise both forms of the line. Was 570
  ...
  memory_stack: 6_350,         // 6,275 — −3, the same three characters `context_block` records (it wraps this stack): the addressing rule, the seed stance and the style-defaults ladder are all re-authored here. The ceiling stands at 1.2% headroom. Was 6,278 — +305, the same two edits `context_block` carries (it wraps this stack): law (b)'s reply-language clause and the `(since <date>)` suffix on every standing rule. Was 6_050 for 5,973 — the mature stack, card through long doc, on a turn that touches none of it (+6 when law (b) started naming the layers that are really in the prompt). Was 5,950 for 5,837; +130 gave the medium tier back its hard-personal-rules line. Was 12,500 for 12,290; the card is +43 on the preamble it replaced and absorbed the tenure section, then the three ladders and the three seed stances came off)
};
 ...
 *
 * The affect compiler takes it to **0.0077** (1,207 against 156,550, down from 157,619), and this is
 * the smallest exchange rate in the list on purpose: a thousand characters of weather prose and
 * seventy-seven of contract bought one four-decimal point. Read it the way the P2 note above asks to
 * be read — what this phase bought is not size, it is that the block now instructs instead of
 * describing. The prose it deleted was under one percent of what the model reads; the persona is
 * still what stands between this number and anything better.
 *
 * The shared persona block takes it DOWN to **0.0073** (1,207 against 163,751, up from 156,550), and
 * this is the largest single fall the floor has recorded. Stated plainly rather than banded away:
 * 7,174 characters of personality moved into Convo's persona head, and the drift anchor grew 27 more,
 * so on THIS surface the phase spent size. What it bought is the thing no size can show from here —
 * the same 7,172 bytes are now what the Composer relay and both Fallfirm voices read too, where the
 * personality used to be four thinner and quietly divergent descriptions. Three surfaces gained a
 * whole person; this one paid for it, and the floor is re-measured under the payment rather than left
 * a phase behind. The prose commit is where the other half comes back: the character sections this
 * block replaces are still standing in Context.md until then, and they come out there.
 *
 * The hook wiring re-measures it at **0.0073** and leaves it exactly there, and the reason is worth
 * a sentence rather than silence. The fixture this floor is measured on is the mature turn, which is
 * a TASK turn — and a task turn is the one shape this whole phase adds nothing to: no hooks section,
 * no `Turn: idle` clause, the same drift anchor. That is the feature's central claim, and this
 * number holding is the closest thing to a measurement of it. The two idle fixtures are where the
 * new sections are measured; neither is a candidate for this floor, because the share of the prompt
 ...
 *
 * The lane-prose commit re-measures it at **0.0088** and it HOLDS (1,207 against 136,555, down from
 * 136,674). Written out rather than left silent, because a floor nobody looked at and a floor that
 * was measured and did not move are different facts. What moved on THIS surface is 119 characters —
 * the tool docs shed 113, the model-map tail 3 and the context block 3 — and 119 of 136,674 is the
 * fifth decimal place. Three of the lines this commit rewrote are invisible from here, each for its
