...
export const PROMPT_BUDGET: Record<BudgetKey, number> = {
  ...
  craft_modules: 29_690,       // 29,589 — the THREAD-OFFER fixture, and a SUM over whichever pages the turn's gates fired rather than the size of any one thing: threading + send-order + reminders + onboarding + email-flag + hooks is the widest set a turn can carry. The Never-Send-a-Leaf rewrite: +39 from 29,660, which is the most misleading number in this table until it is broken into its parts — craft/hooks.md is NEW and 4,561 of it, craft/onboarding.md came out 3,151 lighter and craft/threading.md 1,373 lighter, both of those rewritten in the new register rather than trimmed and both better for it (4,561 − 3,151 − 1,373 = 37; the last two characters are the `\n\n` a ninth page brings with it, personaModules.ts). So the widest craft a turn can carry barely moved while three of the six pages in it changed completely. The fixtures that do NOT load the hook page show the rewrite plainly: the media turn measures 20,234, the mature and long-thread turns 17,569, the cold turn 17,560, the group burst 7,433 — and a turn with no thread, no file and a filled-in profile carries none of it. The hook page had never been in a measured sum before this phase, because it gates on the idle turn (personaModules.ts `idle_turn`) and no fixture used to be idle; the thread-offer fixture had no CHOICE about becoming one, because an offer is only ever made on a hook turn (persona/hooks.ts — `offerAllowed` is true in that mode and no other), so an offer on a task turn is a shape the assembler can no longer build and the widest craft set had been measured on one. Was 29_760 for 29,662 — −73 for the THIRD supervisor corrections, one page and one bullet: craft/send-order.md 3,778 → 3,705, where the passing-mention bullet becomes the rule it was always written for — only an explicit ask is work — which settles their "ok" without first teaching the beat in order to exempt it. Unlike the −39 below this one is not fixture-selective in the other direction: EVERY fixture that loads the page moved the full −73, the mature and long-thread turns from 17,569 to 17,496 among them, because send-order gates on the reply-order section rather than on a tool. Was 29_800 for 29,699 — −37 for the second supervisor corrections, two register fixes on pages this sum carries: craft/reminders.md −39 (the confirmation she writes after setting one is a flat confirming text, and an antisocial hour is stated plainly, once, and set anyway) against craft/send-order.md +2, the one page on this branch that GREW, because the thumbs-up emoji spelled out as "a thumbs-up tapback" is +17 against −14 for a close that is flat rather than warm and −1 for the RIGHT label above it. The two fixtures that carry reminders but not send-order moved the full −39: the cold turn and the group burst. Was 30_700 for 30,371, with T3's 709-character placeholder page standing in for the real one. Was 30_000 for 29,660 — the same widest set before any hook page joined it (8,901 of that the threading page, 2,674 the reminders page); those four other fixtures each rose 322 when the automations paragraph moved off the threading page onto the reminders page, which the MAXIMUM cannot see because the widest fixture loads both. Was 23,400 for the media fixture's 23,100, when the media turn was the widest there was. NOT a prose ratchet like the lines below it: the number moves when a fixture's facts fire a different set, and the pages' own bytes are pinned by the corpus sha256 in personaModules.test.ts
