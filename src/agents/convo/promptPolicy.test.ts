...
test('the JSON anchor states the bubble law with the pipeline constants', () => {
  const { json } = anchors();
  assert.ok(
...
test("the composer's format anchor states the word target and comma-split rule (no count cap)", () => {
  assert.ok(
